...
class RichContentFormatterTests: XCTestCase {
    ...
    func testRemoveGutenbergGalleryListMarkup() {
        ...
        XCTAssertTrue(range.location == NSNotFound)
        range = sanitizedString.range(of: "<figure>")
        XCTAssertTrue(range.location == NSNotFound)
    }
}
